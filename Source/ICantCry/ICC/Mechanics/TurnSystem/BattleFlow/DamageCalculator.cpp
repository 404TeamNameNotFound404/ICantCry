...
int FDamage::CalculateDamage(const bool& IsPlayerAttacking)
{
	...
	if (IsPlayerAttacking)
	{
		...
		const int CeiledResult = FMath::CeilToInt(Result);
		...
		DebugHelper::AddMessageToLog("Risultato approssimato per eccesso (Ceil Result) -> " + FString::SanitizeFloat(CeiledResult));
		
		DebugHelper::LogMessage(3, FColor::FromHex("433878"), "Damage dealt -> " + FString::SanitizeFloat(CeiledResult));
		DebugHelper::AddMessageToLog("Damage dealt -> " + FString::SanitizeFloat(CeiledResult));
		return CeiledResult;
	}
